...
**Key principle:** "Autopilot PROPOSES, merchant APPROVES" — inverted flow that reduces friction while maintaining control.
...
#### Innovation #1: Closed-Loop Profit Automation

Unlike traditional analytics dashboards that show data and leave merchants to figure out actions, Trafi's Profit Engine implements a complete feedback loop:

```
Instrumentation → Diagnosis → Action Proposal → Merchant Approval → Execution → Measurement → Proof/Rollback
```
