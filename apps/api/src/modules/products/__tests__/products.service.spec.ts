...
describe('ProductsService', () => {
  ...
  describe('create', () => {
    ...
    it('should generate product ID with prod_ prefix', async () => {
      prismaService.product.findFirst.mockResolvedValue(null);
      prismaService.product.create.mockImplementation(async ({ data }: { data: { id: string } }) => ({
        ...mockProduct,
        id: data.id,
      }));
      ...
      expect(prismaService.product.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            id: expect.stringMatching(/^prod_[a-f0-9]{24}$/),
          }),
        }),
      );
    });
  ...
  describe('findById', () => {
    ...
    it('should enforce tenant isolation', async () => {
      ...
      expect(prismaService.product.findFirst).toHaveBeenCalledWith({
        where: { id: mockProductId, storeId: 'other_store' },
      });
    });
  });
